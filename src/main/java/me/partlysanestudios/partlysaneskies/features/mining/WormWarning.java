...
public class WormWarning {
    ...
    public void wormWarningChatEvent(ClientChatReceivedEvent event) {
        ...
        if (event.message.getUnformattedText().startsWith("You hear the sound of something approaching...")) {
            if (PartlySaneSkies.Companion.getConfig().getWormWarningBanner()) {
                wormWarningBannerTime = PartlySaneSkies.Companion.getTime();
                wormWarningString = "A Worm Has Spawned!";

                BannerRenderer.INSTANCE.renderNewBanner(new PSSBanner(wormWarningString, (long) (PartlySaneSkies.Companion.getConfig().getWormWarningBannerTime() * 1000), 3, PartlySaneSkies.Companion.getConfig().getWormWarningBannerColor().toJavaColor()));
            }
            if (PartlySaneSkies.Companion.getConfig().getWormWarningBannerSound()) {
