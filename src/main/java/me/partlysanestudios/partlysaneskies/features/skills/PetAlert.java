...
public class PetAlert {
    ...
    public static void runPetAlertTick() {
        if (!PartlySaneSkies.Companion.getConfig().getIncorrectPetForMinionAlert()) {
            return;
        }
        ...
        String selectedPetName = PartlySaneSkies.Companion.getConfig().getSelectedPet();
        
        if (petName.equalsIgnoreCase(selectedPetName)) {
            return;
        }

        if (MathUtils.INSTANCE.onCooldown(lastMuteTime, (long) (PartlySaneSkies.Companion.getConfig().getPetAlertMuteTime() * 60L * 1000L))) {
            return;
        }
        ...
        if (!MathUtils.INSTANCE.onCooldown(lastMessageSendTime,3000)) {
            ...
            "\n§dClick this message or run /mutepetalert to mute the alert for " + PartlySaneSkies.Companion.getConfig().getPetAlertMuteTime() + " minutes.");
    ...
    public static void favouritePet() {
        ...
        PartlySaneSkies.Companion.getConfig().setSelectedPet(petName);
    ...
    public static void registerCommand() {
        new PSSCommand("mutepetalert")
                ...
                .setRunnable((s, a) -> {
                    ChatUtils.INSTANCE.sendClientMessage("§bPet alert has been muted for " +  PartlySaneSkies.Companion.getConfig().getPetAlertMuteTime() + " minutes.");
...
//        Gets the slot where the minion head is supposed to be
    ...
    public void renderInformation(GuiScreenEvent.BackgroundDrawnEvent event) {
        ...
        if (!PartlySaneSkies.Companion.getConfig().getSelectedPetInformation()) {
            return;
        }
        ...
        if (PartlySaneSkies.Companion.getConfig().getSelectedPet().isEmpty()) {
            petColorCode = "§d";
        }
        else if (currentlySelectedPetName.equalsIgnoreCase(PartlySaneSkies.Companion.getConfig().getSelectedPet())) {
            petColorCode = "§a";
