...
public class HelpCommand {
    ...
    private static String getOneConfigKeyBindString() {
        if (PartlySaneSkies.Companion.getConfig().general.oneConfigKeybind.getSize() == 0) {
            return Keyboard.getKeyName(Keyboard.CHAR_NONE);
        }

        String str = Keyboard.getKeyName(PartlySaneSkies.Companion.getConfig().general.oneConfigKeybind.getKeyBinds().get(0));
        for (int i = 1; i < PartlySaneSkies.Companion.getConfig().general.oneConfigKeybind.getSize(); i++) {
            str += " + " +  Keyboard.getKeyName(PartlySaneSkies.Companion.getConfig().general.oneConfigKeybind.getKeyBinds().get(i));
        }
